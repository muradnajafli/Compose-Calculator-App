...
class CalculatorViewModel: ViewModel() {
    ...
    fun onAction(action: CalculatorAction) {
        when (action) {
            ...
            is CalculatorAction.CalculatorOperation -> enterOperation(action.symbol)
    ...
    private fun performDeletion() {
        when {
            ...
            state.operation.isNotBlank() -> state = state.copy(
                operation = ""
            )
    ...
    private fun enterDecimal() {
        if (state.operation.isEmpty() && !state.number1.contains(".")
